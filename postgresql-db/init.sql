-- ============================================================================
-- OnLooker schema for PostgreSQL (local Docker / any Postgres).
--
...
-- ============================================================================
-- PRODUCT PIPELINE (Dapr microservices: embeding-service -> features-extractor
-- -> audience-settings -> develop-analysis). A "product" is the uploaded
-- document the synthetic audience reacts to. The UI stores the raw file here
-- and publishes only {id_product, doc_type} over Dapr.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS audience_responses (
    ...
    id_product             uuid REFERENCES products (id_product) ON DELETE CASCADE,
    id_audience            text,            -- audience group label
    id_feature             text,            -- features[].feature_id
    confident_score        double precision,
    complexity_score       double precision,
    security_score         double precision,
    engagement_score       double precision,
    interest_score         double precision,
    value_perceived_score  double precision,
    general_sentiment_score double precision,
    created_at             timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_audience_responses_product ON audience_responses (id_product);

-- Final analysis per product, produced by develop-analysis.
CREATE TABLE IF NOT EXISTS product_analysis (
    id_product             uuid PRIMARY KEY REFERENCES products (id_product) ON DELETE CASCADE,
    strengths              jsonb,
    weakness               jsonb,
    points_with_potential  jsonb,
    audience_response_analysis text,
    final_recomendations   jsonb,
    aggregate_scores       jsonb,           -- generic % per metric (avg across responses)
    created_at             timestamptz NOT NULL DEFAULT now()
);
