...
async def generate_report(
    ...
    """Generate an email draft and return the PPTX download URL."""
    result = await db.execute(
        select(SessionModel).where(SessionModel.id == session_id)
    )
    session = result.scalar_one_or_none()
    ...
    return {
        "session_id": str(session_id),
        "pptx_download": f"/session/{session_id}/report/download",
        "pptx_available": True,
        "email_draft": email,
    }
...
async def download_report(
    ...
    """Generate and stream the PPTX report directly to the client."""
    result = await db.execute(
        select(SessionModel).where(SessionModel.id == session_id)
    )
    session = result.scalar_one_or_none()
