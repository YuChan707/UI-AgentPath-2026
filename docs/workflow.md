...
scores, anchored in real evidence (local Census grounding) and generated by a
dockerized Llama (llama3.1) via Dapr.
...
flowchart TD
    ...
    subgraph LLM["Model access + grounding"]
        FIQ["grounding.ground(stats)<br/>local evidence (real Census)"]
        CLIENT["LlamaClient<br/>auto: dapr -> http -> mock"]
        DAPR["Dapr Conversation API<br/>component: llama"]
        LLAMA["dockerized Llama (llama3.1)<br/>(OpenAI-compatible)"]
        FIQ --> CLIENT
        CLIENT --> DAPR --> LLAMA
    end
    ...
    P2 -.uses.-> CLIENT
    P3 -.uses.-> CLIENT
    OUTJSON --> AUD["audience-settings<br/>(reactions to a product's features)"]
```
...
flowchart TD
    ...
    GROUND["grounding.ground(stats)<br/>appends real Census evidence to the prompt"]
